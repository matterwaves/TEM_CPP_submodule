...
void calc_pot(struct arrayF3 out_pot, float* coords, void* proton_counts_raw, int atom_count,
              arrayF3 atom_pots, float pixel_size_rc, float pixel_size_z)
{
    ...
    for(int i = 0; i < atom_count; i++) {
        ...
        if(i % 10000 == 0) printf("\r... ... Embedded %d/%d atoms", i, atom_count);
    }
...
void make_solvent_pot(float* out_potential, int shape0, int shape1, int shape2, 
