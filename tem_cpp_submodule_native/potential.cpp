    ...
    int print_progress)
{
    ...
    for(int i = 0; i < atom_count; i++) {
        ...
        if(atom_id < 0 || atom_id > atom_pots_shape0) continue;

        // Get the potential of this atom
        float* atom_pot = &atom_pots[atom_id * atom_pots_shape1 * atom_pots_shape2];
        ...
        int ind0 = (int)roundf(pos0/pixel_size_rc + out_pot_shape0/2.0f);
        int ind1 = (int)roundf(pos1/pixel_size_rc + out_pot_shape1/2.0f);
        int ind2 = (int)roundf(pos2/pixel_size_z  + out_pot_shape2/2.0f);
        
        if(ind2 < 0 || ind2 >= out_pot_shape2) continue;

        int start0 = ind0 - atom_pots_shape1/2;
        int start1 = ind1 - atom_pots_shape2/2;

        // These are the bounding indicies of where we will do our computation
        int s0 = max_c(start0, 0);
        int e0 = min_c(start0 + atom_pots_shape1, out_pot_shape0);

        int s1 = max_c(start1, 0);
        int e1 = min_c(start1 + atom_pots_shape2, out_pot_shape1);
        ...
        for(int i0 = s0; i0 < e0; i0++) {
            ...
            for(int i1 = s1; i1 < e1; i1++) {
                ...
                out_pot[(i0*out_pot_shape1 + i1)*out_pot_shape2 + ind2] += atom_pot[my_ind0 * atom_pots_shape2 + my_ind1];
            }
        }
