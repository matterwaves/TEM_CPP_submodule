...
cdef extern from "wrapper.h":
    void calc_pot(
        float* out_pot, int out_pot_shape0, int out_pot_shape1, int out_pot_shape2,
        float* coords, void* proton_counts_raw, int atom_count, float* atom_pots,
        int atom_pots_shape0, int atom_pots_shape1, int atom_pots_shape2,
        float pixel_size_rc, float pixel_size_z, int print_progress);
...
def generate_protein_potential(
            ...
            tuple pot_shape, 
            bytes atom_pots, 
            tuple atom_pots_shape, 
            float pixel_size_rc, 
            float pixel_size_z,
            int print_progress
    ):
    assert len(pot_shape) == 3, "pot_shape must be a 3-tuple"
    assert len(atom_pots_shape) == 3, "atom_pots_shape must be a 3-tuple"
    ...
    assert len(atom_pots) == (atom_pots_shape[0] * atom_pots_shape[1] * atom_pots_shape[2] * sizeof(float)), "atom_pots must have a size euqal to atom_pots_shape * sizeof(float)"
